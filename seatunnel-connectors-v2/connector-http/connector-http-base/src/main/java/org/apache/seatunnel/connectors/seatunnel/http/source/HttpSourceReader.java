...
import static org.apache.seatunnel.connectors.seatunnel.http.constants.encryptConstant.ACCESS_KEY;
import static org.apache.seatunnel.connectors.seatunnel.http.constants.encryptConstant.DATA;
import static org.apache.seatunnel.connectors.seatunnel.http.constants.encryptConstant.ENCRYPT_BIZ_CONTENT;
...
public class HttpSourceReader extends AbstractSingleSplitReader<SeaTunnelRow> {
    ...
    public void pollAndCollectData(Collector<SeaTunnelRow> output) throws Exception {
        ...
        if (!Optional.ofNullable(this.httpParameter.getParamsConvert())
                        .map(List::isEmpty)
                        .orElse(true)
                || !Optional.ofNullable(this.httpParameter.getBodyConvert())
                        .map(List::isEmpty)
                        .orElse(true)) {
        ...
        if (StringUtils.isNotBlank(body)) {
            try {
                ...
                DocumentContext jsonContext =
                        JsonPath.using(Configuration.defaultConfiguration()).parse(body);
                isBodyArrays =
                        CollectionUtils.isNotEmpty(bodyPListKey)
                                && bodyPListKey.stream()
                                        .allMatch(key -> isKeyPresent(jsonContext, key));
            } catch (Exception e) {
                throw new SeaTunnelException("body转json失败，请保证body是正常的json字符串", e);
            }
        }
    ...
    private void updateListBody(
        ...
        for (String jsonPathKey : bodyPListKey) {
            ...
            if (value instanceof JSONArray) {
                valueList = ((List<Object>) value).stream().collect(Collectors.toList());
                updateBody.put(jsonPathKey, valueList);
            } else {
                throw new SeaTunnelException(
                        "Value for key "
                                + jsonPathKey
                                + " is not an Array. 请检查您的json数据，可能是由于您的数组被双引号包裹起来了");
            }
        }
        // 根据相同索引拼接每个参数体
        ...
        if (CollectionUtils.isEmpty(valueList)) {
            ...
            for (int i = 0; i < valueList.size(); i++) {
                DocumentContext singleUpdateBodyNode =
                        JsonPath.using(Configuration.defaultConfiguration())
                                .parse(jsonContext.jsonString());

                for (String jsonPathKey : bodyPListKey) {
                    String[] keys = jsonPathKey.split("\\.");
                    String rightKey = keys[keys.length - 1];
                    String leftKey =
                            jsonPathKey.substring(0, jsonPathKey.length() - rightKey.length() - 1);
    ...
    private void updateListParams(
            Collector<SeaTunnelRow> output,
            String url,
            Map<String, String> params,
            String body,
            List<String> paramsListKey,
            Map<String, String> headers)
            throws Exception {
    ...
    private Map<String, String> paramsConvertTime(List<String> convert, Map<String, String> map)
            throws Exception {
        if (convert == null) {
            return map;
        }
                            ...
                            try {
                                ...
                                localDateTime =
                                        TimeUtils.parseDateTime(
                                                timeStr,
                                                TimeUtils.Formatter.YYYY_MM_DD_HH_MM_SS_SSS);
                            } catch (Exception e1) {
                                // 如果失败，尝试使用不带毫秒的格式解析日期时间字符串
                                localDateTime =
                                        TimeUtils.parseDateTime(
                                                timeStr, TimeUtils.Formatter.YYYY_MM_DD_HH_MM_SS);
                            }
    ...
    private Map<String, String> headerConvertTime(List<String> convert, Map<String, String> map)
            throws Exception {
        if (convert == null) {
            return map;
        }
    ...
    private String addQuotesToJsonValues(String json) {
        // Regex to find all values in square brackets ([]), that are not already surrounded by
        // quotes
        String regex = "\\[(.*?)\\]";
            ...
            if (entry.getValue().isObject()) {
                ...
                if (keys.contains(entry.getKey())) {
                    ...
                    try {
                        ...
                        localDateTime =
                                TimeUtils.parseDateTime(
                                        timeString, TimeUtils.Formatter.YYYY_MM_DD_HH_MM_SS_SSS);
                    } catch (Exception e1) {
                        // 如果失败，尝试使用不带毫秒的格式解析日期时间字符串
                        localDateTime =
                                TimeUtils.parseDateTime(
                                        timeString, TimeUtils.Formatter.YYYY_MM_DD_HH_MM_SS);
                    }
                    long timeStamp = TimeUtils.toEpochSecond(localDateTime);
    ...
    private void checkUrl(
        ...
        if (!hasListUrl) {
            ...
            String[] pathParamsList = url.split("\\[")[1].replace("]", "").split(",");
    ...
    private void executeRequest(
        ...
        Map<String, String> encryptParams =
                handler.handleParams(params, this.httpParameter.getParamsEncrypt());
        String encryptBody =
                handler.handleBody(body, this.httpParameter.getBodyEncrypt(), encryptParams);
        Map<String, String> encryptHeaders =
                handler.handleHeader(headers, this.httpParameter.getHeadersEncrypt(), encryptBody);
        ...
        log.info(
                "实际请求后的返回结果： request url:[{}], request headers:[{}] ,request param:[{}], request body:[{}], http response status code:[{}], content:[{}]",
                url,
                encryptHeaders,
                encryptParams,
                encryptBody,
        ...
        if (response.getCode() >= 200 && response.getCode() <= 207) {
            ...
            if (MapUtils.isNotEmpty(this.httpParameter.getParamsEncrypt())) {
                ...
                if ("RSA".equals(this.httpParameter.getParamsEncrypt().get(ENCRYPT_TYPE))) {
                    ...
                    if (StringUtils.isBlank(responseString)) {
                        log.error(
                                "接口请求异常： request url:[{}], request headers:[{}], request param:[{}], request body:[{}],http response status code:[{}], content:[{}]",
                                url,
                                encryptHeaders,
                                encryptParams,
                                encryptBody,
                        ...
                        .equals(this.httpParameter.getParamsEncrypt().get(ENCRYPT_TYPE))) {
                    ...
                    if (Boolean.parseBoolean(
                            this.httpParameter.getParamsEncrypt().get(ENCRYPT_BIZ_CONTENT))) {
                        try {
            ...
            content =
                    processAddition(
                            content, context, paramsAdd, paramsJsonNode, paramsPath, "params");
            // headers追加
            JsonNode headersJsonNode = JsonUtils.toJsonNode(headers);
            content =
                    processAddition(
                            content, context, headersAdd, headersJsonNode, headersPath, "headers");
            // body追加
            JsonNode bodyJsonNode =
                    JsonUtils.stringToJsonNode(StringUtils.isBlank(body) ? "" : body);
            ...
            String error =
                    String.format(
                            "接口请求异常, request url:%s, request headers:%s, request param:%s, request body:%s,http response status code:%s, content:%s",
                            url,
                            encryptHeaders,
                            encryptParams,
                            encryptBody,
    ...
    void handleJsonNode(
            String parentPath, DocumentContext context, String prefix, JsonNode jsonNode) {
        ...
        while (fields.hasNext()) {
            ...
            } catch (com.jayway.jsonpath.InvalidModificationException e) {
                log.error(
                        String.format(
                                "响应体非map结构而是array结构，导致无法追加params、headers、body、cookies到响应体：%s",
                                context.jsonString()),
                        e);
            }
        }
     ...
     * @throws JsonProcessingException
     */
    private String processAdditionCookies(
            String content, DocumentContext context, boolean isAdd, String cookies, String dataPath)
            throws JsonProcessingException {
    ...
    private void checkAndUpdateNodePage(JsonNode node, String pageField, Long index)
            throws Exception {
