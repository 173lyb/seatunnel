...
public class HttpParameter implements Serializable {
    ...
    public void buildWithConfig(Config pluginConfig) {
        ...
        if (pluginConfig.hasPath(HttpConfig.SKIP_SSL_VERIFICATION.key())) {
            this.setSkipSslVerification(pluginConfig.getBoolean(HttpConfig.SKIP_SSL_VERIFICATION.key()));
        } else{
        ...
        if (pluginConfig.hasPath(HttpConfig.PARAMS_CONVERT.key())) {
            this.setParamsConvert(
                    pluginConfig.getStringList(HttpConfig.PARAMS_CONVERT.key()));
        }
        // set body_conversion
        if (pluginConfig.hasPath(HttpConfig.BODY_CONVERT.key())) {
            this.setBodyConvert(
                    pluginConfig.getStringList(HttpConfig.BODY_CONVERT.key()));
        }
        //headersConvert
        if (pluginConfig.hasPath(HttpConfig.HEADERS_CONVERT.key())) {
            this.setHeadersConvert(
                    pluginConfig.getStringList(HttpConfig.HEADERS_CONVERT.key()));
        }
        // set params_ban_loop
        if (pluginConfig.hasPath(HttpConfig.PARAMS_BAN_LOOP.key())) {
            this.setParamsBanLooP(
                    pluginConfig.getStringList(HttpConfig.PARAMS_BAN_LOOP.key()));
        }
